...
1. **Hold** **SUPER + Z** — the wheel opens under your cursor while the
   key is down.
2. Flick toward an item and **release** — it runs. Any flick works,
   however short: the release resolves to the item in that direction
   even before the cursor leaves the hub. Releasing on a category opens
   its sub-wheel instead; hold and release again to pick from it.
   Releasing **without flicking** (in place) goes **back** one level —
   at the root wheel it opens the settings editor — and releasing off
